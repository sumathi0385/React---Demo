...
function App() {
  ...
  moviedata.map((d) => console.log(d.name));

  return <div className="App">{<Main moviedata={moviedata} />}</div>;
}
...
function Movies({ title, image, rate, text }) {
  return (
    <div className="col-3">
      <img className="small" src={image} alt={title} />
      <div className="row titleinfo">
        <header>{title}</header>
        <header>{rate}</header>
      </div>
